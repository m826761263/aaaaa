<template>
    <div>
        <van-swipe-cell v-for="v in list" :key="v.id">
            <van-checkbox v-model="checked" class="cricle">{{ v.shopName }}
                <div>{{ v.time }}</div>
                <van-icon name="arrow" />
            </van-checkbox>
...
>>>.van-checkbox__label>div{
    margin: 0 10px;
    color: var(--miao-font-color);
    border: 1px solid var(--miao-font-color);
    font-size: 5px;
    padding: 1px 5px;
}
>>>.van-card__num{
